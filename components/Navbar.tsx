    ...
    <nav className="sticky top-0 z-50 bg-bg-alpha backdrop-blur-md border-b border-border transition-colors duration-300">
        ...
        <div className="hidden md:flex items-center gap-0.5">
          {routes.map((route) => (
            <a
              key={route.href}
              href={route.href}
              className="font-mono text-[11px] px-2.5 py-2 whitespace-nowrap transition-colors text-text-muted hover:text-accent"
            >
              {route.name}
            </a>
          ))}
      ...
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 ${
          menuOpen ? "max-h-[24rem] border-t border-border" : "max-h-0"
        }`}
      >
